...
object LanguageCacheManager {
    ...
    fun targetLanguage(): String {
        ...
        return if (supportedCodes.contains(code)) code else "en"
    }

    fun selected(context: Context): Set<String> {
        val saved = context.getSharedPreferences(PREFS, Context.MODE_PRIVATE)
            .getStringSet(KEY_LANGUAGES, null)
        return saved?.toSet() ?: setOf("en")
    }

    fun saveSelected(context: Context, codes: Set<String>) {
        context.getSharedPreferences(PREFS, Context.MODE_PRIVATE)
            .edit().putStringSet(KEY_LANGUAGES, codes).apply()
    }

    fun prepareSelected(context: Context, callback: (done: Int, total: Int, failed: Int) -> Unit = { _, _, _ -> }) {
        val required = (selected(context) + targetLanguage()).toList().distinct()
